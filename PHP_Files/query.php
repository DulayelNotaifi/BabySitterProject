...
// babysitter
function get_babysitter_email($email)
{
    global $conn;
...
function babysitter_signup_handler($email, $password, $fname, $lname, $gender, $id, $age,  $city, $phone,  $msg, $img)
{

    global $conn;
    $sql = "INSERT INTO `babysitter` VALUES ('" . $email . "','" . $password . "','" . $fname . "','" . $lname . "','" . $gender . "','" . $id . "','" . $age . "','" . $city . "','" . $phone . "','" . $msg . "','" . $img . "')";
    $result = mysqli_query($conn, $sql);
...
{
    global $conn;
    $sql = "SELECT requests.TypeOfServese, requests.startTime, requests.endTime, requests.startDate, requests.startDate,kids.kidName, kids.kidAge, offers.price, offers.offerstatus FROM `requests`, `offers`, `kids` WHERE requests.ID = kids.ID AND requests.ID = offers.RequestID AND offers.babySitterEmail = '" . $email . "'";
    $result = mysqli_query($conn, $sql);
    return $result;
}
